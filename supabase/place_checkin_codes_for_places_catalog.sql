...
-- (STA table column is "id"; the catalog view aliases it as establishment_public_id)
-- Safe to re-run. Paste into Supabase SQL Editor.

-- 1) Seed codes only for STA rows (valid FK targets)
...
-- 2) Resolve check-in code by places.id (name → STA id) OR direct STA id
