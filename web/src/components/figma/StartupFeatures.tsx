...
function Dots({ active }: { active: number }) {
  ...
  const dots = [0, 1, 2, 3];
  return (
    <div className="ft-dots" role="group" aria-label="Carousel progress">
      {dots.map((i) => (
        <div
          key={i}
          className={`ft-dot ${i < active ? 'ft-dot--active' : ''}`}
          aria-hidden="true"
        />
      ))}
    </div>
...
function StartupFeatureCard({ feature }: { feature: StartupFeature }) {
  return (
    <article className="ft-startupCard" aria-label={feature.title} role="article">
      ...
      <div className="ft-startupCardInner">
        ...
        <div className="ft-startupButtonWrap">
          <button
            type="button"
            className="ft-primaryBtn"
            aria-label={`Get started: ${feature.title}`}
          >
            GET STARTED
          </button>
        </div>
...
export function StartupFeatures() {
  const features = useMemo<StartupFeature[]>(
    () => [
      {
        id: 'explore',
  ...
  return (
    <section className="ft-screen ft-startupScreen" aria-label="Startup Features">
      <div className="ft-startupRow" role="region" aria-label="Feature cards">
        {features.map((f) => (
          <StartupFeatureCard key={f.id} feature={f} />
        ))}
      </div>
    </section>
  );
