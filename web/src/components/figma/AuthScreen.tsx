...
export function AuthScreen({ variant }: { variant: AuthVariant }) {
  ...
  return (
    <main className="ft-screen ft-authScreen" aria-label="Authentication screen">
      <section className="ft-authCard" role="region" aria-label={title}>
        <header className="ft-authHeader">
          <Link
            to="/"
            className="ft-iconBtn ft-iconBtn--back"
            aria-label="Back"
            title="Back"
          >
        ...
        <form onSubmit={onSubmit} className="ft-authForm" aria-describedby={`${formId}-error`}>
          ...
          <div className="ft-authRow">
            {isSignIn ? (
              <div className="ft-authRowLeft">
                <button type="button" className="ft-linkBtn" aria-label="Forgot Password?">
                  Forgot Password?
                </button>
              </div>
            ) : (
              <label className="ft-checkbox" aria-label="Remember device">
                <input
                  type="checkbox"
                  checked={remember}
          ...
          <button
            type="submit"
            disabled={loading}
            className="ft-primaryBtn ft-primaryBtn--auth"
          ...
          <button
            type="button"
            className="ft-secondaryBtn"
            aria-label={isSignIn ? 'Log in with Google' : 'Sign up with Google'}
            onClick={() => alert('Google sign-in is not wired in this prototype yet.')}
          >
          ...
          <p className="ft-authFooter">
            ...
            {isSignIn ? (
              <Link to="/prototype/sign-up" className="ft-authFooterLink" aria-label="Sign up">
                Sign up
              </Link>
            ) : (
              <Link to="/prototype/sign-in" className="ft-authFooterLink" aria-label="Sign in">
                Sign in
              </Link>
            )}
          </p>
