...
export function getAllDestinationReachedEntries(userId) {
  const uid = String(userId ?? '').trim();
  if (!uid) return [];
  const all = readAll();
  const prefix = `${uid}:`;
...
export function getAllDestinationReachedCount(userId) {
  return getAllDestinationReachedEntries(userId).length;
}
